...
\subsection{Minimal data requirements}

\mesc\ requires the following data as input to work:
\begin{itemize}
	\item spatial metric $\gam_{ij}$
	\item extrinsic curvature $K_{ij}$
	\item lapse function $\alp$
	\item rest-frame density $\rho$ (as defined in \texttt{HydroBase})
	\item 3--velocity $v^i$ (as defined in \texttt{HydroBase})
\end{itemize}
Please ensure you have the appropriate 3D output from the ET before proceeding.
