...
contains
  ...
  !
  subroutine send_for_analysis(nx,ntimes,nargs,times,xmax,xmin,dx,gij,kij,alp,vel0,vel1,vel2,rho,dens)
    use options, only:nspheres,radius,looprad,radmin,radmax,drad
    use ricci,   only:compute_ricci

    integer, intent(in) :: nx,ntimes,nargs !nargs is number of command line arguments [to know when to write a header to files]
    ...
    do while (rad<=radend)
       ...
       call compute_ricci(nx,ntimes,nargs,dx,xmax(1),xmin(1),rad,nsph,gij,kij,rho,&
