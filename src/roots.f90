module roots
  ...
  !
  !       - Ricci tensor components (and fluidR calc)
  !       - Christoffel symbols
  !       - Backreaction terms and cosmological parameters
  !       - Expansion \Theta and shear scalar \sigma^2 etc.
  !
  use options, only:fourth,pi,c_double,dit,tinitial,dtfac,dp,&
       & tderivs,nord_dt,writeQdterms
  use spines, only:calc_average_within_radius,calc_od_ud_vol,&
       & calc_average_within_radius_manyspheres
  use manipulations, only:get_metric_at_pos,inv3x3,calc_raised_comp_4D,calc_up_down
  use derivatives, only:deriv1,deriv1fourth,deriv1_bckwrd,deriv1_bckwrd2nd,&
       & deriv1_bckwrd3rd,deriv1_bckwrd4th,deriv2fourth,deriv2_bckwrd2nd,&
       & deriv2,deriv2_mix,deriv2_mixfourth
...
contains
  ...
  subroutine get_ricci_component(i,j,k,Chrsijk,nx,dx,ridx1,ridx2,riccic)
    ...
    real(c_double), intent(in) :: Chrsijk(3,3,3,nx,nx,nx),dx
    ...
    do l=1,3
       ...
       if (fourth) then
          ...
          term1 = term1 + deriv1(gamma1_p1,gamma1_m1,dx)
       endif
       ...
       if (fourth) then
          ...
          term2 = term2 + deriv1(gamma2_p1,gamma2_m1,dx)
       endif
  ...
  subroutine get_christoffel(i,j,k,gij,nx,dx,idx1,idx2,idx3,chr)
    ...
    do l=1,3
       if (fourth) then
          ...
          trm1 = deriv1(g3dum_p1(l), g3dum_m1(l), dx)  ! d_{j} (g_{k,l})
          trm2 = deriv1(gdum2_p1(l), gdum2_m1(l), dx)  ! d_{k} (g_{l,j})
          trm3 = deriv1(g23p1(l), g23m1(l), dx)  ! d_{l} (g_{j,k})
       endif
  ...
  subroutine get_backreaction_omegas(it,time,rad,nspheres,randorigins,avgs,lenavgs,rhoavgall,rhoavgall_notilde,&
    ...
    if (writeQdterms) then
       call write_avg(avgs(5,:),"theta2_avg",it,time,nspheres,rad,"\eta, <\Theta^2>",randorigins)
       call write_avg(avgs(4,:),"sigma2_avg",it,time,nspheres,rad,"\eta, <\sigma^2>",randorigins)
       call write_avg(avgs(6,:),"omega2_avg",it,time,nspheres,rad,"\eta, <\omega^2>",randorigins)
    endif
  ...
  subroutine get_fluid_curvature(nx,nt,dx,dt,ipos,jpos,kpos,gdown_atstencil,gup,kij,&
    ...
    real(c_double) :: dtuUi(3),diuUt(3),uUt,aU0
    real(c_double) :: alpijk,RUUsum,nabu_nabu,fourR00
    real(c_double) :: term1,term2a(3),term2b(3),term3a(3,3),term3b(3,3)

    integer :: i,j,k

    call get_4D_ricci_tensor(nx,nt,dx,dt,ipos,jpos,kpos,gdown_atstencil,gup,kij,&
       ...
       do i=1,3
          ...
          case(1)
             ...
             if (fourth) then
                ...
                diuUj(i,j) = deriv1(fvelUip1(j),fvelUim1(j),dx)
             endif
          ...
          case(2)
             ...
             if (fourth) then
                ...
                diuUj(i,j) = deriv1(fvelUjp1(j),fvelUjm1(j),dx)
             endif
          ...
          case(3)
             ...
             if (fourth) then
                ...
                diuUj(i,j) = deriv1(fvelUkp1(j),fvelUkm1(j),dx)
             endif
          end select
       enddo
    enddo
    ...
    do k=1,3
       ...
       do j=1,3
          ...
          term2a(k) = term2a(k) + lorentz * gup(k,j) * dialp(j) - alpijk * kud(k,j) * fvelU(j)
  ...
  !      --> ONLY called if tderivs .and. notearly
  !
  subroutine get_4D_ricci_tensor(nx,nt,dx,dt,ipos,jpos,kpos,gdown_atstencil,gup,kij,&
    ...
    real(c_double) :: dtKxx,kUdKddsum(3,3)
    ...
    trkijk = tracek(ipos,jpos,kpos,nt)
    trkip1 = tracek(ip1,jpos,kpos,nt)
    trkim1 = tracek(im1,jpos,kpos,nt)
    trkjp1 = tracek(ipos,jp1,kpos,nt)
    trkjm1 = tracek(ipos,jm1,kpos,nt)
    trkkp1 = tracek(ipos,jpos,kp1,nt)
    trkkm1 = tracek(ipos,jpos,km1,nt)
    ...
    if (fourth) then
       ...
       trkip2 = tracek(ip2,jpos,kpos,nt)
       trkim2 = tracek(im2,jpos,kpos,nt)
       trkjp2 = tracek(ipos,jp2,kpos,nt)
       trkjm2 = tracek(ipos,jm2,kpos,nt)
       trkkp2 = tracek(ipos,jpos,kp2,nt)
       trkkm2 = tracek(ipos,jpos,km2,nt)
    endif
    ...
    if (fourth) then
       ...
       diK(1)   = deriv1(trkip1,trkim1,dx) ! \pd_x K
       diK(2)   = deriv1(trkjp1,trkjm1,dx) ! \pd_y K
       diK(3)   = deriv1(trkkp1,trkkm1,dx) ! \pd_z K
    endif
    ...
    if (fourth) then
       ...
       didjalp(1,1) = deriv2(alpip1,alpijk,alpim1,dx) ! d2x alp
       didjalp(2,2) = deriv2(alpjp1,alpijk,alpjm1,dx) ! d2y alp
       didjalp(3,3) = deriv2(alpkp1,alpijk,alpkm1,dx) ! d2z alp

       didjalp(1,2) = deriv2_mix(ipos,jpos,nx,alp(:,:,kpos),dx) ! dxdy alp
       didjalp(1,3) = deriv2_mix(ipos,kpos,nx,alp(:,jpos,:),dx) ! dxdz alp
       didjalp(2,3) = deriv2_mix(jpos,kpos,nx,alp(ipos,:,:),dx) ! dydz alp
    endif
    ! apply symmetries
    ...
    do n=1,3
       ...
       if (fourth) then
          ...
          dxgup = deriv1(gupip1(1,n),gupim1(1,n),dx) ! \pd_x g^{xi}
          dygup = deriv1(gupjp1(2,n),gupjm1(2,n),dx) ! \pd_y g^{yi}
          dzgup = deriv1(gupkp1(3,n),gupkm1(3,n),dx) ! \pd_z g^{zi}

          dxKud = deriv1(kudip1(1,n),kudim1(1,n),dx) ! \pd_x K^x_i
          dyKud = deriv1(kudjp1(2,n),kudjm1(2,n),dx) ! \pd_y K^y_i
          dzKud = deriv1(kudkp1(3,n),kudkm1(3,n),dx) ! \pd_z K^z_i
       endif
  ...
  !     as defined in Buchert+2019 -- using Hamiltonian-type like constraint (4.16)
  !
  ! \mathcal{R} = 16\pi G \rho - 2/3 \Theta^2 + 2 \sigma^2 - 2 w^2
  !
  ...
  !
  ! S subroutine to get the shear w.r.t the projection vector b_mu,nu = g_mu,nu + u_mu u_nu
  !     and the fluid expansion \Theta (or \theta if tderivs=False)
  !     and the vorticity w_munu
  !
  ! * NOTE * this routine uses a linear approximation for \Theta and \sigma (true
  !          to a factor of v^2) if we have set tderivs=.False.
  ...
  subroutine get_expansion_shear_vort(ipos,jpos,kpos,nx,nt,dx,dt,notearly,gij_atstencil,&
    ...
    real(c_double), intent(out), optional :: wddmunu(4,4)
    !
    ...
    real(c_double) :: deltaij,diudj(3,3)                ! \delta^i_j, \partial_i u_j, and \partial_i u_t
    ...
    real(c_double) :: gamjji,gamkij,gamilm,diuUi
    !
    ! things we need at stencil for spatial derivatives
    !
    real(c_double) :: alpijk,alpip1,alpim1,alpjp1,alpjm1,alpkp1,alpkm1
    real(c_double) :: alpip2,alpim2,alpjp2,alpjm2,alpkp2,alpkm2
    real(c_double) :: dxalp,dyalp,dzalp,dxuUx,dyuUy,dzuUz
    ...
    real(c_double), dimension(3) :: udip1,udim1,udjp1,udjm1,udkp1,udkm1
    ...
    if (fourth) then
       ...
       ! \partial_i u^i
       dxuUx = deriv1(uUip1(1),uUim1(1),dx)
       dyuUy = deriv1(uUjp1(2),uUjm1(2),dx)
       dzuUz = deriv1(uUkp1(3),uUkm1(3),dx)

       ! \partial \alp
       dxalp = deriv1(alpip1,alpim1,dx)
       dyalp = deriv1(alpjp1,alpjm1,dx)
       dzalp = deriv1(alpkp1,alpkm1,dx)
    endif
    ...
    do jj=1,3
       ...
       case(1)
          ...
          if (fourth) then
             ...
             diudt(jj) = deriv1(udtip1,udtim1,dx)
          endif
       case(2)
          ...
          if (fourth) then
             ...
             diudt(jj) = deriv1(udtjp1,udtjm1,dx)
          endif
       case(3)
          ...
          if (fourth) then
             ...
             diudt(jj) = deriv1(udtkp1,udtkm1,dx)
          endif
       end select
       ...
       do ii=1,3
          ...
          case(1)
             ...
             if (fourth) then
                ...
                diudj(ii,jj) = deriv1(udip1(jj),udim1(jj),dx)
             endif
          ...
          case(2)
             ...
             if (fourth) then
                ...
                diudj(ii,jj) = deriv1(udjp1(jj),udjm1(jj),dx)
             endif
          ...
          case(3)
             ...
             if (fourth) then
                ...
                diudj(ii,jj) = deriv1(udkp1(jj),udkm1(jj),dx)
             endif
    ...
    dtudt = 0._dp
    if (tderivs .and. notearly) then
       dtudt = - lorentz * dtalp + 0.5_dp * udt * lorentz**2 * dtvsq
    endif
    ...
    do ii=1,3 ! component loop, sig,w components are ii+1
       ...
       do jj=1,3
          ...
          if (tderivs .and. notearly) then
             ...
             dwnwmunu(ii+1,jj+1)   = 0._dp ! this is zero to 3rd order
          endif
       enddo
    enddo
