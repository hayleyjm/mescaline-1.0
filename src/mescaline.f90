...
! Written by Hayley Macpherson &  Daniel Price, 2017-2021
!
!    mescaline-1.0: first public version
!
!---------------------------------------------------------------------
program mescaline
  use cactoidae, only:read_cactus_file
  use analysis,  only:send_for_analysis
  use prints,    only:print_mescaline,print_info,print_error
 ...
 real(c_double), allocatable, dimension(:,:,:,:) :: vel0,vel1,vel2
       ...
       if (tderivs) then
          ...
          if (i >= nfiles_toread) then
              ...
              if (first) then
                  ...
                  do n=1,nfiles
                      ...
                      if (n==1) then
                        ...
                        if (allocated(kij)) deallocate(kij,gij,vel0,vel1,vel2)
                        allocate(gij(6,nx,nx,nx,nfiles),kij(6,nx,nx,nx,nfiles),vel0(nx,nx,nx,nfiles),&
                             & vel1(nx,nx,nx,nfiles),vel2(nx,nx,nx,nfiles))
                     endif
                     gij(:,:,:,:,n) = gijn
                     kij(:,:,:,:,n) = kijn
                     vel0(:,:,:,n)  = vel0n
                     ...
                     deallocate(gijn,kijn,vel0n,vel1n,vel2n)
                  enddo
              ...
              if (restart) then
                   ...
                   if (allocated(kij)) deallocate(kij,vel0,vel1,vel2)
                   allocate(gij(6,nx,nx,nx,nfiles),kij(6,nx,nx,nx,nfiles),vel0(nx,nx,nx,nfiles),&
                      & vel1(nx,nx,nx,nfiles),vel2(nx,nx,nx,nfiles))
          ...
          !
          ! allocate memory for all-time arrays
          if (allocated(kij)) deallocate(gij,kij,vel0,vel1,vel2)
          allocate(gij(6,nx,nx,nx,nfiles),kij(6,nx,nx,nx,nfiles),vel0(nx,nx,nx,nfiles),&
          & vel1(nx,nx,nx,nfiles),vel2(nx,nx,nx,nfiles))
