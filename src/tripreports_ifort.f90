module tripreports
  ...
  !
  ! --> This version specifically for ifort compilation. Difference is only in open statements for writing arrays of length (nspheres) in write_avg
  !      since nspheres can be large. This currently works for 1000 spheres (tried 5000 and DID NOT work. Be careful.)
  !          (we have a seperate file here because we don't want to affect the behaviour when using gfortran, which will be the majority of cases)
  !
...
contains
  ...
  subroutine write_avg(quantity,ident,it,time,nspheres,radius,header,randorigins)
    ...
    reclen = 100000 ! long record length for large numbers of spheres
    open(newunit=unit,file=filename,status='replace',recl=reclen)
    call print_info(" Writing to "//trim(filename),loc)
  ...
  ! to write 3D data in 2D (x-y) slices to an ascii file
  !
  !   INCLUDING ghost cells specified by nghosts in options.f90
  !     ghost cells are written as zero-valued - the ET fills these in periodically
  !      for one 3D array of data and ONE filename
  !
  !  ** currently only called from ``make ics''
  !
  subroutine write_3d_data_withghosts(nx,filename,data,time)
    integer, intent(in) :: nx
    character(len=*), intent(in) :: filename
    real(c_double), intent(in) :: data(nx,nx,nx)
    real(c_double), intent(in), optional :: time

    real(c_double), allocatable :: datagh(:,:,:)
    integer :: i,k,ntot
    integer :: unt
    ntot = nx + 2 * nghosts ! total number of grid cells
    allocate(datagh(ntot,ntot,ntot))

    call print_info(" Writing to "//trim(filename)//" (with ghost cells)",loc)
    !print*, ' and adding ',nghosts*2,' zero-valued ghost cells. '
    ! open file + timestamp
    open(newunit=unt,file=filename,status='replace')
    if (present(time)) write(unt,*) time

    ! put data into ghost array
    ! surely we can do this all in one loop...
    datagh = 0.
    do k=nghosts+1,ntot-nghosts
       do i=nghosts+1,ntot-nghosts
          datagh(i,nghosts+1:ntot-nghosts,k) = data(i-nghosts,:,k-nghosts)
          !write(unt,*) datagh(i,:,k)
       enddo
    enddo
  ...
  end subroutine write_3d_data_withghosts
